  ...
  return (
    <div className='flex-1 flex flex-row justify-between'>
      <div className='flex flex-col'>
        ...
        <p className="tet-gray-600">#{props.orderDetail.id}</p>
      </div>
      <div className='flex flex-row gap-2 items-end'>
        {props.orderDetail.salesChannel !== '-' &&
          <Badge className="bg-gray-300 rounded-full text-white hover:bg-gray-300 h-fit text-xs font-semibold">
            <p className='text-xs font-semibold text-black text-center'>{props.orderDetail.salesChannel.toUpperCase()}</p>
          </Badge>}
